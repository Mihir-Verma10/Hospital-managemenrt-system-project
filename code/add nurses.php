...
<form method="POST">
<h1 style="position:absolute;top:120px;left:50px;color:black;font-family:'algerian">ADD NEW NURSE IN HOSPITAL</h1>
<input type="text" name="id" placeholder="GIVE ID TO NURSE" style="position:absolute;top:360;left:50px;height:35px;width:215px" required>
...
$email=$_POST['email'];
$id=$_POST['id'];
if(empty($na) or empty($mobile) or empty($address) or empty($email) or empty($id))
{
...
{
$query1="select * from nurses where email=$email;";
$q1=mysqli_query($conn,$query1);
$num=mysqli_num_rows($q1);
	if($num>=1)
	{
	echo'<h3 style="position:absolute;top:460px;left:50px;color:red;font-family:algerian"><u><blink>'."the nurse is already in your hospital"."</blink></h3>";
	}
	else
	{

	$query="insert into nurses values('$id','$na','$address',$mobile,'$email');";
$q=mysqli_query($conn,$query);
		if($q)
		{
	echo'<h3 style="position:absolute;top:460px;left:100px;color:red;font-family:algerian"><u><blink>'."nurse added"."</blink></h3>";
